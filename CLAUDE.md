...
- **命名規則**: PEP 8 に従う。クラス名は PascalCase（例: `DBBackup`）、関数名・変数名は snake_case（例: `get_connection`）、定数は UPPER_SNAKE_CASE（例: `BACKUP_DIR`）。
- **フォーマット**: インデントはスペース 4 つ。最大行長は 79 文字を推奨。
...
- **Discord 通知**: `notify(config, "タイトル", 0x008000, "メッセージ")` を使用（第 1 引数は Config、第 3 引数は色を表す int）

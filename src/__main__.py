...
class FullBackup(BaseBackup):
    ...
    def process(self,
        ...
        log(LOG_FILE, "[INFO] Starting backup")
        command = f"./rsync.sh -h '{config.FULL_HOSTNAME}' -r '{config.FULL_PORT}' -u '{config.FULL_USERNAME}' -i '{config.FULL_IDENTITY}' -p '{config.FULL_PASSPHRASE}' -f '{config.FULL_FROM}' -o '{BACKUP_DIR}' -l '{LOG_FILE}' 2>&1 | tee {LOG_FILE}"
        print(command)
